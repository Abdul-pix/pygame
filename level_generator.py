...
class Game:
    ...
    Het spel heeft 4 toestanden:
        'menu'   : startscherm
        'playing': spel bezig
        'dead'   : speler verloren
        'won'    : speler heeft finish bereikt
    """

    def __init__(self):
        """
        Initialiseer pygame, scherm, fonts
        en laad het eerste level.
        """
        pygame.init()
    ...
    def reset(self):
        """
        Genereer een nieuw level en
        maak alle spelobjecten opnieuw aan.
        """
        (
            self.platforms,
            self.finish,
    ...
    def draw(self):
        ...
        for turret in self.turrets:
            turret.draw(self.screen, self.camera_x)
    ...
    def draw_msg(self, title, ondertitel):
        ...
        self.screen.blit(
            grote_tekst,
            (SCREEN_W // 2 - grote_tekst.get_width() // 2, 230)
        )

        self.screen.blit(
            kleine_tekst,
            (SCREEN_W // 2 - kleine_tekst.get_width() // 2, 300)
        )
